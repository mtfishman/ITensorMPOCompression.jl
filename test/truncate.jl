...
@testset verbose=verbose "Truncate/Compress" begin
    ...
    @testset "Truncate/Compress iMPO Check gauge relations, H=$(model[1]), ul=$ul, qbs=$qns, N=$N, NNN=$NNN" for model in models, ul in [lower], qns in [false,true], N in [1,2,3,4], NNN in [1,2,4,7]
        initstate(n) = "↑"
        makeH=model[1]
        site_type=model[2]
