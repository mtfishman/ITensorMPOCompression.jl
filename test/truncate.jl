...
@testset "Truncate/Compress iMPO Check gauge relations, H=$(test_combo[1]), ul=$ul, qbs=$qns" for test_combo in test_combos, ul in [lower], qns in [false]
    initstate(n) = "↑"
    makeH=test_combo[1]
    ...
    for  NNN in [1], N in [1] #3 site unit cell fails inside ITensorInfiniteMPS for qns=true.
        ...
        #
        for n in 1:N
            #@show Ss[n-1]*HR[n] HL[n]*Ss[n] Ss[n-1]*HR[n]-HL[n]*Ss[n]
            @test norm(Ss[n-1]*HR[n]-HL[n]*Ss[n]) ≈ 0.0 atol = eps_gauge
        end    
        #
